...
Sequel.migration do
  ...
  up do
    create_table(:agency_descendant) do
      primary_key :id

      Integer :agent_corporate_entity_id, :null => false
      Integer :descendant_id, :null => false
    end

    alter_table(:agency_descendant) do
      add_foreign_key([:agent_corporate_entity_id], :agent_corporate_entity, :key => :id)
      add_foreign_key([:descendant_id], :agent_corporate_entity, :key => :id)
    end
