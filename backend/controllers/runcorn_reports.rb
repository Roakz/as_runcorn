class ArchivesSpaceService < Sinatra::Base
  ...
  do
    [
        ...
        {
            "Content-Type" => "text/csv",
            "Content-Disposition" => "attachment; filename=\"conservation_requests.#{Date.today.iso8601}.csv\""
        },
        ConservationRequestsReport.new(DateParse.date_parse_down(params[:from_date]), DateParse.date_parse_up(params[:to_date])).to_stream
    ]
  end
  ...
  do
    [
        ...
        {
            "Content-Type" => "text/csv",
            "Content-Disposition" => "attachment; filename=\"assessments.#{Date.today.iso8601}.csv\""
        },
        AssessmentsReport.new(DateParse.date_parse_down(params[:from_date]), DateParse.date_parse_up(params[:to_date])).to_stream
    ]
  end
  ...
  do
    [
        ...
        {
            "Content-Type" => "text/csv",
            "Content-Disposition" => "attachment; filename=\"conservation_treatments.#{Date.today.iso8601}.csv\""
        },
        ConservationTreatmentsReport.new(DateParse.date_parse_down(params[:from_date]), DateParse.date_parse_up(params[:to_date])).to_stream
    ]
  end
end
